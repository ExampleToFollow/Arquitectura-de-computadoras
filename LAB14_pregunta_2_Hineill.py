...
def bruto_paralelo( vocal , abecedario, aux):
		for j in vocales:
			for h in abecedario:
				if (not aux.value):
					password = vocal+j+h
...
if __name__ == "__main__":
	...
	bruto_serial (abecedario)
	toc = time.perf_counter()
	time_serial = toc-tic
	print(f"Tiempo de ejecucion es { time_serial}")
	tic = time.perf_counter()
	manager=Manager()
	aux = manager.Value('b',False)
	p= Pool(processes =5)
	args= [(i,abecedario,aux) for i in vocales]
	p.starmap(bruto_paralelo, args)
	p.close()
	p.join()
	toc = time.perf_counter()
	time_paralelo= toc-tic
	print(f"El tiempo de ejecucion en paralelo es de { time_paralelo}")
